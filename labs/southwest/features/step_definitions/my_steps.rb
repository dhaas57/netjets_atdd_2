...
When(/^I search for flights departing and arriving at the same airport$/) do
  ...
  @test_departure_airport, @test_arrival_airport = search_for_flights_between_same_airport
end

Then(/^I am notified that I cannot perform the search$/) do
  # expect(departure_error_message).to include 'Invalid route'
  expect(departure_error_message).to_not be_nil
end